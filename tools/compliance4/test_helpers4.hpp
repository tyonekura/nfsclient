...
// Assert that `expr` evaluates to true; throw ComplianceFailure4 otherwise.
#define CHECK4(expr) \
    do { \
        if (!(expr)) { \
            throw compliance4::ComplianceFailure4( \
                "CHECK4 failed: " #expr " at " __FILE__ ":" + std::to_string(__LINE__)); \
        } \
    } while (0)
...
#define EXPECT_NFS4_ERR(expr, code) \
    do { \
        try { \
            (expr); \
            throw compliance4::ComplianceFailure4( \
                "EXPECT_NFS4_ERR(" #code "): no exception thrown"); \
        } catch (const Nfs4Error& _nfs4_e) { \
            if (!_nfs4_e.is(code)) { \
                throw compliance4::ComplianceFailure4( \
                    std::string("EXPECT_NFS4_ERR(" #code "): got nfsstat4=") \
